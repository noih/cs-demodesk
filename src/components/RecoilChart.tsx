          ...
          {points.length === 0 && reference.length === 0 ? <Flex align="center" justify="center" style={{ minHeight: 180 }}><Text size="2" color="gray">{t('recoil.empty')}</Text></Flex> : <>
            <Box className="recoil-plot"
              ...
              <EChart height="100%" option={{
                ...
                series: [reference, points].map((path, index) => ({ type: 'line', z: index + 2, data: (index === 0 ? visible.reference : visible.player) ? path.map(p => [p.x, p.y]) : [], symbol: 'circle', symbolSize: index === 0 ? 5 : 4, showSymbol: true,
                  ...
                  label: { show: true, position: 'right', color: colors.text, textShadowColor: colors.panel, textShadowBlur: 3, textShadowOffsetY: 1, formatter: (p: { dataIndex: number }) => p.dataIndex === 0 || (p.dataIndex + 1) % 5 === 0 ? String(p.dataIndex + 1) : '' },
                })),
              }} />
            </Box>
