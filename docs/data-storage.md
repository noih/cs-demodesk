...
Anomaly videos use the existing highlight render queue and renderer. A manual export selects one player and saved assessment plus observed rule IDs. By default each selected rule produces a separate queued job with merging enabled. The optional cross-rule merge creates one chronological video: overlapping padded windows for the same player and round become one interval from the earliest start to the latest end, retaining all rule tags. Chained overlaps are joined too; separate intervals stay separate. The preview uses the same merged windows for its clip count and duration. The job stores its clip snapshot in `analysisClips`, so later analysis does not change a queued export. Each rule video retains 3 seconds before the first event and after the last event, and 1.5 seconds on the internal sides of intervening clips, bounded by the available timeline. The source content fingerprint is checked before recording. Per-rule statistics, normal highlight selection and automatic basic analysis remain independent.
