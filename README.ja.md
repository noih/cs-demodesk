...
スコア、各ラウンドの結果、プレイヤー別スタッツ（K/D/A、ADR、ヘッドショット、ユーティリティダメージ、マルチキル、クラッチ）に加え、ラウンドのタイムラインと比較チャート。
...
- 録画中の CS2 は既定で非表示です。書き出し設定の「ゲーム画面を表示」で表示できます。1 つの Steam アカウントで同時に起動できる CS2 は 1 つだけなので、録画中はゲームをプレイできません。書き出しジョブは 1 つずつ実行され、残りはキューに入ります。
...
- チャットアプリやブラウザで再生するなら H.264 を選んでください。NVIDIA エンコーダーには NVIDIA GPU が必要です。

## 設計

データベースを持たず、内部状態を極力持ちません。すべては実行ファイルの隣の `demodesk-data\` にある単純なファイルです。設定と書き出しジョブは JSON、解析結果・リプレイストリーム・レーダー画像はバージョン付きのキャッシュで、いつでも削除でき、デモ・スキーマ・ゲームのバージョンが変わると再生成されます。デモ一覧は更新のたびにリプレイフォルダを走査するので、アプリの外でファイルを追加・移動・削除しても副作用はありません。
