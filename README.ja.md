...
試合結果とプレイヤーのパフォーマンスを、交戦、ユーティリティの使用、ラウンドの状況から振り返れます。比較チャート、ラウンドごとの推移、リコイルコントロールの軌跡で試合を多角的に確認できます。

![AK-47、M4A4、M4A1-S のリコイルコントロールの軌跡](docs/images/ja/03-combat-analysis.png)

### 異常データ（実験的機能）

全プレイヤーの照準・射撃・移動の統計や、TTD、推定スモーク越し命中率などを確認できます。プレイヤーと行動項目ごとに気になるクリップを選択・結合・書き出して、詳しく見直せます。

統計は参考情報であり、チートの判定ではありません。意図的に隠され、統計に異常が出ないチートは識別できず、異常がなくても不正がないとは限りません。

![異常データ（実験的機能）](docs/images/ja/05-anomaly-data.png)
...
![2D リプレイ](docs/images/ja/07-2d-view.png)
