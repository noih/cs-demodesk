...
fn build_window_hook(out: &std::path::Path) {
    ...
    let bytes = if let Some(path) = env::var_os("DEMODESK_DETOURS_PACKAGE") {
        ...
        ureq::get("https://codeload.github.com/microsoft/Detours/zip/refs/tags/v4.0.1")
            .call()
            .expect("download Detours (or set DEMODESK_DETOURS_PACKAGE)")
            .body_mut()
            .read_to_vec()
            .unwrap()
    };
    ...
    for i in 0..zip.len() {
        ...
        let Some(name) = entry.name().strip_prefix("Detours-4.0.1/src/") else {
            continue;
        };
    ...
    command
        .current_dir(out)
        .args(["/LD", "/EHsc", "/DWIN32_LEAN_AND_MEAN"])
    ...
    let result = command
        ...
        .arg(format!(
            "/OUT:{}",
            out.join("demodesk-window-hook.dll").display()
        ))
        .output()
        .expect("compile native window hook");
    assert!(
        result.status.success(),
        "window hook compilation failed: {} {}",
    ...
    let result = compiler
        ...
        .arg(format!(
            "/Fe:{}",
            out.join("demodesk-window-probe.exe").display()
        ))
        .arg("/link")
        .arg("user32.lib")
        .output()
        .expect("compile window probe");
    assert!(
        result.status.success(),
        "window probe compilation failed: {} {}",
        String::from_utf8_lossy(&result.stdout),
        String::from_utf8_lossy(&result.stderr)
    );
}
