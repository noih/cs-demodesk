...
mod platform {
    ...
    impl ProcessTree {
        ...
        pub fn finish(&self) -> io::Result<()> {
            ...
            unsafe {
                checked(TerminateJobObject(self.0.as_raw_handle(), 1))?;
            }
                ...
                match unsafe { WaitForSingleObject(process.as_raw_handle(), remaining) } {
                    ...
                    WAIT_TIMEOUT => {
                        return Err(io::Error::new(
                            ...
                            "output process exit not signaled",
                        ))
                    }
        ...
        fn start(&self, command: &Command, capture: Option<(&File, &File)>) -> io::Result<Child> {
            ...
            let (output, error) = capture
                .map(|(out, err)| (out.as_raw_handle(), err.as_raw_handle()))
            ...
            unsafe {
                ...
                let result = checked(CreateProcessW(
                    program.as_ptr(),
                    line.as_mut_ptr(),
                    null(),
                    null(),
    ...
    impl Child {
        pub fn kill(&mut self) -> io::Result<()> {
            unsafe { checked(TerminateProcess(self.0.as_raw_handle(), 1)) }
        }
        pub fn try_wait(&mut self) -> io::Result<Option<ExitStatus>> {
            unsafe {
                match WaitForSingleObject(self.0.as_raw_handle(), 0) {
                    ...
                    WAIT_OBJECT_0 => {
                        let mut code = 0;
                        checked(GetExitCodeProcess(self.0.as_raw_handle(), &mut code))?;
        ...
        pub fn wait(&mut self) -> io::Result<ExitStatus> {
            ...
            self.try_wait()?
                .ok_or_else(|| io::Error::other("process did not exit"))
        }
    }
}
#[cfg(windows)]
...
mod tests {
    ...
    fn command(role: &str, dir: &Path) -> Command {
        ...
        cmd.args(["--exact", "render::process::tests::fixture", "--nocapture"])
            .env("DEMODESK_PROCESS_TEST_ROLE", role)
            .env("DEMODESK_PROCESS_TEST_DIR", dir)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null());
    ...
    #[test]
    fn fixture() {
        let Ok(role) = std::env::var("DEMODESK_PROCESS_TEST_ROLE") else {
            return;
        };
        ...
        if role == "echo" {
            ...
            println!(
                "env={}",
                std::env::var("DEMODESK_PROCESS_TEST_VALUE").unwrap()
            );
        ...
        let tree = if role == "owner" {
            ...
            if role == "branch" {
                leaf = Some(command("leaf", &dir).spawn().unwrap());
            }
            None
        };
    ...
    fn closing_owner_or_crashing_it_kills_descendants_only() {
        for abrupt in [false, true] {
            ...
            assert_eq!(
                unsafe { WaitForSingleObject(outside.as_raw_handle(), 0) },
                WAIT_TIMEOUT
            );
    ...
    fn real_recording_tools_use_managed_processes() {
        ...
        assert!(
            !crate::render::record::is_process_running("cs2.exe").unwrap(),
            "close CS2 first"
        );
        let cs2 = std::env::var_os("DEMODESK_TEST_CS2").expect("DEMODESK_TEST_CS2");
        let hlae = std::path::PathBuf::from(
            std::env::var_os("DEMODESK_TEST_HLAE").expect("DEMODESK_TEST_HLAE"),
        );
        ...
        let mut console = loop {
            if let Ok(socket) = std::net::TcpStream::connect((std::net::Ipv4Addr::LOCALHOST, port))
            {
                break socket;
            }
            assert!(
        ...
        console
            ...
            .unwrap();
        console
            .write_all(b"echo demodesk_process_tree_ready\n")
            .unwrap();
        ...
        assert!(
            tree.contains_pid(game_pid).unwrap(),
            "CS2 did not inherit the job"
        );
        ...
        tree.finish().unwrap();
        assert_eq!(
            unsafe { WaitForSingleObject(game_handle.as_raw_handle(), 0) },
            WAIT_OBJECT_0,
            "CS2 exit signal"
        );
        assert!(!crate::render::record::is_process_running("cs2.exe").unwrap());
        let output = ProcessTree::new()
            .unwrap()
            .output(Command::new(ffmpeg).args([
                "-hide_banner",
                "-f",
                "lavfi",
                "-i",
