...
fn paths(cs2_dir: &Path) -> Paths {
    ...
    Paths {
        csdm_dir: csgo.join("csdm"),
        gameinfo: csgo.join("gameinfo.gi"),
        backup: csgo.join("gameinfo.gi.backup"),
    }
}
...
pub(super) fn has_leftovers(cs2_dir: &Path) -> bool {
    let p = paths(cs2_dir);
    p.csdm_dir.exists()
        || fs::read_to_string(&p.gameinfo)
            .map(|c| c.contains(SEARCH_PATH_LINE))
            .unwrap_or(false)
}
...
pub(super) fn remove_leftovers(cs2_dir: &Path) -> Result<()> {
    ...
    if p.backup.exists() {
        ...
        if content.contains(SEARCH_PATH_LINE) {
            fs::write(
                &p.gameinfo,
                content.replacen(&format!("{SEARCH_PATH_LINE}\n\t\t\t"), "", 1),
            )?;
        }
    }
    Ok(())
