...
fn main() -> Result<()> {
    let a: Vec<_> = std::env::args().skip(1).collect();
        ...
        a.len() == 6,
        "expected STATE PARSED GAME VRF CACHE NEW_OUTPUT"
    );
    ...
    for (i, kill) in kills.iter().enumerate() {
        ...
        for t in (tick - 16).max(0)..=tick + 3 {
            windows.entry(t).or_default().push(i);
        }
    }
    ...
    let coverage = native_body::visit(
        Path::new(&a[0]),
        &prepared,
        &prepared.skeleton,
        |tick, frame| {
            if let Some(indices) = windows.get(&tick) {
                for &i in indices {
                    let kill = &kills[i];
                    let attacker = frame.iter().find(|p| {
                        Some(p.player_id.as_str()) == kill["attacker"]["steamid"].as_str()
                    });
                    let victim = frame
                        .iter()
                        .find(|p| Some(p.player_id.as_str()) == kill["victim"]["steamid"].as_str());
        ...
        samples.iter().any(|s| !s.is_empty()),
        "no kill participants matched the native player IDs"
    );
    ...
    serde_json::to_writer(
        output,
        &json!({"pointNames":prepared.point_names,"coverage":coverage,"kills":kills,"samples":samples}),
    )?;
