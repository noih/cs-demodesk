...
fn main() -> Result<()> {
    ...
    let frames = if args.len()==6 {DemoParser::new().player_tracking_frames(&bytes,args[2].parse()?,args[3].parse()?,args[4].parse()?)?} else {scene::extract(
        &DemoParser::new(),
        &bytes,
        args[2].parse()?,
        args[3].parse()?,
    ...
    let result = demodesk_core::analysis::Artifact {
        contract: if args.len()==6 {demodesk_core::analysis::Contract {module:"player-tracking-scene".into(),schema_version:1,implementation_version:"0.1.0".into()}} else {scene::contract()},
