...
![Траектории отдачи AK-47, M4A4 и M4A1-S](docs/images/ru/03-combat-analysis.png)

### Аномальные данные (экспериментальная функция)

Анализируйте всех игроков вручную и экспортируйте клипы действий. Функция экспериментальная, статистика — только для справки. Она не выявляет намеренно скрытые читы без статистических аномалий; отсутствие аномалий не означает отсутствие читов.

![Аномальные данные (экспериментальная функция)](docs/images/ru/05-anomaly-data.png)
...
- По умолчанию окно CS2 во время записи скрыто. Чтобы показать его, отключите «Скрывать игру и запускать в фоне» в настройках экспорта.
- Один аккаунт Steam может запускать только одну CS2, поэтому во время записи играть нельзя. Задачи экспорта выполняются по одной, остальные ждут в очереди.
...
- Для воспроизведения в мессенджерах и браузерах выбирайте H.264.
- Кодировщикам NVIDIA нужна видеокарта NVIDIA.

## Устройство

База данных не используется. Настройки, задачи экспорта и кэши анализа хранятся в файлах, по умолчанию в локальном каталоге данных пользователя `%LOCALAPPDATA%\dev.noih.demodesk\demodesk-data`. Папку данных можно изменить в настройках. Демо остаются на исходных местах: их можно добавлять по одному или находить сканированием выбранных папок.
