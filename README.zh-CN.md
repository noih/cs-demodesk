...
统计全场玩家的瞄准、射击与移动行为，查看 TTD、估算穿烟命中率等数据，并按玩家与行为项目选择、合并及导出可疑片段，方便回看。

统计仅供参考，不是作弊判定；无法识别刻意掩饰且数据无异常的外挂，没有异常也不代表没有作弊。

![异常数据（实验性）](docs/images/zh-CN/05-anomaly-data.png)
